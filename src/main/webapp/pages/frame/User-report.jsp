...
<script language="javascript">
	...
	function printAuth(value,row,index){
		if(row.tp == 'report'){
			...
			var chked2 = row[this.field+"2"];
			return "<input type=\"checkbox\" "+(chked==1?"disabled=\"true\"":"")+" "+(chked==1||chked2==1?"checked":"")+" value=\"y\" name=\""+this.field+"@"+(row.id - 100000)+"\">";
		}
	}
	...
	function getReportIds(){
		var pageObj = v${mvInfo.formId};
		document.forms[pageObj.formId].elements[pageObj.fromId].value = "frame.UserPermission-reportList";
		document.forms[pageObj.formId].elements[pageObj.midKey].value = "userReportAdd";
